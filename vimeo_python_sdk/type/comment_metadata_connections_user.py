# coding: utf-8

"""
    Vimeo API

    Build something great. Vimeo's API supports flexible, high-quality video integration with your custom apps.

    The version of the OpenAPI document: 3.4
    Created by: https://developer.vimeo.com/help
"""

from datetime import datetime, date
import typing
from enum import Enum
from typing_extensions import TypedDict, Literal, TYPE_CHECKING

from vimeo_python_sdk.type.picture import Picture

class RequiredCommentMetadataConnectionsUser(TypedDict):
    # The absolute URL of the authenticated users's profile page.
    link: str

    # The display name of the user who posted the reply.
    name: str

    # Information about the user's portraits.
    pictures: Picture

    # The unique identifier to access the user resource.
    uri: str

class OptionalCommentMetadataConnectionsUser(TypedDict, total=False):
    pass

class CommentMetadataConnectionsUser(RequiredCommentMetadataConnectionsUser, OptionalCommentMetadataConnectionsUser):
    pass
